...
import { SPREADSHEET_API_URL } from "./services/api";
...
export default function App() {
  ...
  useEffect(() => {
    const initData = async () => {
      try {
        // Try testing connection first to get spreadsheet name
        try {
          const testRes = await fetch(`${SPREADSHEET_API_URL}?action=test`);
          const testResult = await testRes.json();
          if (testResult.success) {
            setSsName(testResult.spreadsheetName || "");
          }
        } catch (e) {
  ...
  const handleDeleteBranch = (id: string) => {
    ...
    const branchName = editingBranch?.name || "chi nhánh này";
    if (window.confirm(`Bạn có chắc chắn muốn xóa "${branchName}"? Tất cả phòng thuộc chi nhánh này cũng sẽ bị xóa.`)) {
      try {
        const updatedBranches = hotelService.deleteBranch(id);
        ...
        if (selectedBranchId === id) {
          setSelectedBranchId(updatedBranches.length > 0 ? updatedBranches[0].id : "");
        }
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (user) {
      ...
      setLoginError("Sai tên đăng nhập hoặc mật khẩu. Nếu bạn mới cập nhật Google Sheets, hãy thử nút 'Tải lại dữ liệu' bên dưới.");
    }
  };
            ...
            <form onSubmit={handleLogin} className="p-8 space-y-6">
              {isLoadingData && (
                <div className="p-4 bg-blue-50 border border-blue-200 rounded-2xl flex items-center gap-3">
                  ...
                  <p className="text-xs font-bold text-blue-700 uppercase tracking-widest">Đang tải dữ liệu từ Google Sheets...</p>
                </div>
              )}
                ...
                <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl space-y-2">
                  <div className="flex items-start gap-2 text-amber-700">
                    ...
                    <div className="text-xs">
                      <p className="font-bold">Lỗi kết nối Google Sheets</p>
                      <p className="opacity-80">{apiError.message}</p>
                    </div>
                  </div>
                  {apiError.debugUrl && (
                    <a 
                      ...
                      target="_blank" 
                      rel="noreferrer"
                      className="block w-full py-2 bg-amber-600 text-white text-[10px] font-bold text-center rounded-lg uppercase tracking-wider"
                    >
                      Mở link chẩn đoán của Google
                    </a>
                  )}
                  {apiError.isAuthError && (
                    <p className="text-[10px] text-amber-600 italic">
                      Lưu ý: Bạn cần cấu hình Web App ở chế độ "Anyone" (Bất kỳ ai).
                    </p>
                  )}
                </div>
              )}
                  ...
                  <div className="px-1 flex flex-col gap-1">
                    <div className="flex items-center gap-2">
                       ...
                       <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">
                         Đã kết nối: {employees.length} nhân viên
                       </p>
                    </div>
                    {ssName && (
                      <p className="text-[8px] text-slate-400 italic truncate pl-3.5">
                        File: {ssName}
                      </p>
                    )}
                  </div>
              ...
              <div className="pt-4 flex items-center justify-between gap-4 border-t border-slate-100">
                <button
                  ...
                  className="flex-1 py-3 bg-slate-50 hover:bg-slate-100 text-slate-500 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2"
                >
                  <RefreshCw size={14} />
                  Tải lại dữ liệu
                </button>
                <button
                  type="button"
                  onClick={() => {
                    if (window.confirm("Bác có chắc muốn xóa sạch bộ nhớ cache? (Không ảnh hưởng dữ liệu trên Google Sheets)")) {
                      localStorage.clear();
    ...
    <div className="flex flex-col h-screen bg-slate-50 font-sans text-slate-900 overflow-hidden relative">
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
          ...
          <div className="flex items-center gap-2 lg:gap-4">
            ...
            {currentUser?.role === 'staff' && (
              <button 
                ...
                className="flex items-center gap-2 px-3 py-2 sm:px-4 sm:py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-[10px] sm:text-xs font-bold rounded-xl transition-all shadow-lg shadow-indigo-100"
              >
                <Scan className="w-4 h-4" />
                <span className="hidden xs:inline">Chấm công QR</span>
              </button>
            )}
        ...
        <div className="flex-1 overflow-y-auto p-4 lg:p-8 bg-slate-50 pb-24 lg:pb-8">
          <AnimatePresence>
            {isAttendanceOpen && currentUser && (
              <AttendanceScanner 
                employee={employees.find(e => e.id === currentUser.username) || { id: currentUser.username, name: currentUser.fullName } as any}
                userBranch={branches.find(b => b.id === selectedBranchId) || null}
          ...
          {activeTab === "rooms" && (
            <div className="max-w-[1600px] mx-auto space-y-12">
              ...
              {branches.length > 0 ? (
                ...
                // If there are branches defined but none exist in the branches list, show all rooms collectively
                <div className="space-y-4">
                  <div className="flex items-center justify-between border-b border-slate-200 pb-2">
                    <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col items-center justify-center py-20 text-slate-400">
                  ...
                  <p className="text-sm font-medium">Chưa có phòng nào được thiết lập</p>
                  <button 
                    onClick={() => setActiveTab('settings')}
                    className="mt-4 px-4 py-2 bg-indigo-600 text-white rounded-lg text-xs font-bold uppercase tracking-widest"
                  >
                    Thiết lập ngay
                  </button>
                </div>
              ...
              {/* Show unassigned rooms if branches exist but some rooms don't match */}
              {branches.length > 0 && selectedBranchId === "all" && roomsWithStatus.some(r => !branches.find(b => b.id === r.branchId)) && (
                 <div className="space-y-4 mt-8 pt-8 border-t border-dashed border-slate-200">
                    <div className="flex items-center justify-between border-b border-slate-200 pb-2">
                      <div className="flex items-center gap-3">
                        <HelpCircle size={20} className="text-slate-400" />
                        <h2 className="text-xl font-black text-slate-400 tracking-tight">
                          Phòng chưa phân loại chi nhánh
                        </h2>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-4 opacity-80">
                      {roomsWithStatus.filter(r => !branches.find(b => b.id === r.branchId)).map((room) => (
                        <RoomCard
                          key={room.id}
                          room={{ ...room, status: room.displayStatus }}
                          onClick={() => handleOpenBooking(room)}
                        ...
                        <button
                          onClick={() => {
                            if (
                              ...
                              const updatedBranches = hotelService.deleteBranch(branch.id);
                              setBranches(updatedBranches);
                              setRooms(hotelService.getRooms());
              ...
              <form onSubmit={handleSaveBranch} className="p-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  ...
                  <div className="space-y-6">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          type="number"
                          step="any"
                          defaultValue={editingBranch?.lat}
                          placeholder="VD: 11.94"
                          className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all font-semibold"
                        />
                      </div>
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          type="number"
                          step="any"
                          defaultValue={editingBranch?.lng}
                          placeholder="VD: 108.44"
                          className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all font-semibold"
                        />
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      onClick={() => {
                        navigator.geolocation.getCurrentPosition((pos) => {
                          ...
                          if (latInput) latInput.value = pos.coords.latitude.toString();
                          if (lngInput) lngInput.value = pos.coords.longitude.toString();
                        });
                      }}
                      className="w-full py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2"
                    >
                      <MapPin size={14} />
                      Lấy vị trí hiện tại
                    </button>
                    ...
                    {editingBranch && (
                      <div className="p-4 bg-indigo-50 rounded-2xl flex flex-col items-center gap-3">
                        <p className="text-[10px] font-bold text-indigo-600 uppercase tracking-widest text-center">Mã QR Chi nhánh Chấm công</p>
                        <div className="bg-white p-3 rounded-xl shadow-sm">
                          <QRCodeSVG value={editingBranch.id} size={140} />
                        </div>
                        <p className="text-[9px] text-indigo-400 text-center italic">Yêu cầu nhân viên quét mã này để ghi nhận sự hiện diện</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="pt-10 flex gap-4">
                  {editingBranch && (
                    <button
                      ...
                      className="px-6 py-4 bg-rose-50 text-rose-600 rounded-2xl font-black uppercase tracking-widest hover:bg-rose-100 transition-all flex items-center justify-center gap-2"
                    >
                      <Trash2 size={20} />
                      Xóa
                    </button>
                  ...
                  <button
                    type="submit"
                    className="flex-1 py-4 bg-indigo-600 text-white rounded-2xl font-black uppercase tracking-widest shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition-all flex items-center justify-center gap-2"
                  >
                    <Check size={20} />
                    Lưu thông tin
                  </button>
